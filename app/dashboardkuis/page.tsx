...
export default function DashboardKuisPage() {
  const [kelasTarget, setKelasTarget] = useState<string>("5"); // Default ke Kelas 5
  ...
  // Cek apakah mode Kelas 5 / 6 (Kuis Mengetik / WPM)
  const isWpmMode = kelasTarget === "5" || kelasTarget === "6";
  ...
  // Fetch Soal yang tersimpan di DB
  ...
  // Fetch Live Results berdasarkan Kelas
  const fetchResults = async () => {
    // Urutkan berdasarkan WPM tertinggi jika Kelas 5/6, atau Durasi Tercepat jika Kelas 4
    let query = supabase
      .from("kuis_jawaban")
      .select("*")
      .eq("kelas", kelasTarget)
    ...
    if (isWpmMode) {
      ...
      query = query.order("durasi_detik", { ascending: true });
    }

    const { data, error } = await query;

    if (error) {
      console.error("Gagal mengambil hasil kuis:", error.message);
    } else if (data) {
      setLiveResults(data);
    }
  };
  ...
  const handleEditExistingSoal = () => {
    ...
    const formatted = existingSoal.map((s) => ({
      acak: s.kalimat_acak || s.teks_panjang || "",
  ...
  const handleDeleteSingleSoal = async (id: string) => {
    if (confirm("Yakin ingin menghapus soal ini?")) {
      ...
      if (error) alert("Gagal menghapus soal: " + error.message);
      else fetchExistingSoal();
    }
  };
  ...
  const handleDeleteAllSoal = async () => {
    if (
      ...
      if (error) alert("Gagal menghapus semua soal: " + error.message);
      else fetchExistingSoal();
    }
  };
  ...
  const handleSubmitSoal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = soalList.map((item) => ({
        ...
        teks_panjang: isWpmMode ? item.acak : null, // Support mode Teks Panjang untuk Kelas 5 & 6
      }));
      ...
      if (insertError) {
        ...
        alert(
          `✅ Berhasil memperbarui soal untuk Kelas ${kelasTarget} (Pertemuan ${kodePertemuan} - Part ${partTarget})!`
        );
  ...
  const handleClearAllResults = async () => {
    if (
      confirm(
        `Hapus SEMUA hasil monitoring untuk Kelas ${kelasTarget} (Pertemuan ${kodePertemuan} - Part ${partTarget})?`
      )
    ) {
      await supabase
        .from("kuis_jawaban")
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-8 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-800 pb-4 gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Kelola soal bertahap per Kelas, Pertemuan &amp; Part
          </p>
        </div>

        <div className="flex flex-wrap items-center gap-3 bg-slate-900 border border-slate-800 p-2 rounded-xl">
          {/* Filter Kelas */}
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <option value="5">Kelas 5 (WPM)</option>
              <option value="6">Kelas 6 (WPM)</option>
            </select>
          </div>
        ...
        <div className="lg:col-span-7 bg-slate-900 border border-slate-800 p-6 rounded-xl space-y-4">
          <div className="flex justify-between items-center border-b border-slate-800 pb-3">
            <h2 className="text-lg font-semibold text-white">
              Input / Edit Soal {isWpmMode ? "(Paragraf Ketik)" : "(Kalimat)"}
            </h2>
            <div className="flex gap-2">
              <span className="bg-sky-400/10 text-sky-400 border border-sky-400/20 text-xs px-2.5 py-1 rounded-md font-bold">
                Kelas {kelasTarget}
              </span>
          ...
          <form onSubmit={handleSubmitSoal} className="space-y-4">
            {soalList.map((soal, idx) => (
              <div
                key={idx}
                className="p-4 bg-slate-800/60 rounded-lg border border-slate-700/50 space-y-3"
              >
                ...
                <div>
                  <label className="text-[11px] text-slate-400">
                    {isWpmMode
                      ? "Masukkan Teks Paragraf Soal Ketik:"
                      : "Kalimat/Teks Awal Soal:"}
                  </label>
                  {isWpmMode ? (
                    <textarea
                      rows={4}
                      placeholder="Masukkan paragraf teks yang harus diketik oleh siswa..."
                      value={soal.acak}
                      onChange={(e) =>
                        handleInputChange(idx, "acak", e.target.value)
                      }
                      required
                      className="w-full bg-slate-900 border border-slate-700 p-2 rounded text-sm text-white mt-1 font-mono"
                    />
                  ) : (
                    <input
                      type="text"
                ...
                {!isWpmMode && (
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="Kunci jawaban benar"
                      value={soal.benar}
                      onChange={(e) =>
                        handleInputChange(idx, "benar", e.target.value)
                      }
                      required
                      className="w-full bg-slate-900 border border-slate-700 p-2 rounded text-sm text-white mt-1"
                    />
            ...
            <div className="flex gap-3">
              ...
              >
                {loading
                  ? "Menerbitkan..."
                  : `Terbitkan (Kelas ${kelasTarget} - P${kodePertemuan} - Part ${partTarget})`}
              </button>
            </div>
          </form>
        </div>
            ...
            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
              {existingSoal.map((item, index) => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-200 font-mono line-clamp-3">
                    <span className="text-slate-400 font-sans">Teks Soal:</span>{" "}
                    {item.teks_panjang || item.kalimat_acak}
                  </p>
                </div>
              ))}
            </div>
      ...
      {/* Tabel Live Monitoring Leaderboard */}
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-xl space-y-4">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-lg font-semibold text-white">
              🔴 Live Leaderboard {isWpmMode ? "Ketik Cepat (WPM)" : "Waktu Tercepat"} (Kelas {kelasTarget} - Pertemuan {kodePertemuan} - Part {partTarget})
            </h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Data otomatis diperbarui realtime secara otomatis.
            </p>
          </div>
          ...
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-800 text-slate-400 uppercase">
              <tr>
                ...
                <th className="p-3">Sub-Kelas</th>
                {isWpmMode ? (
                  <>
                    <th className="p-3">WPM</th>
                    <th className="p-3">Akurasi</th>
                    <th className="p-3">Keystroke (Kasar/Koreksi)</th>
                  </>
                ) : (
            ...
            <tbody className="divide-y divide-slate-800">
              {liveResults.length === 0 ? (
                <tr>
                  <td
                    colSpan={isWpmMode ? 8 : 7}
                    className="p-4 text-center text-slate-500"
                  >
                    Belum ada data pengerjaan pada Kelas {kelasTarget} - Pertemuan {kodePertemuan} - Part {partTarget}...
                  </td>
                </tr>
              ) : (
                liveResults.map((res, index) => (
                  <tr key={res.id} className="hover:bg-slate-800/40">
                    ...
                    <td className="p-3">
                      <span className="px-2 py-0.5 bg-slate-800 border border-slate-700 rounded text-[11px] font-bold text-sky-400">
                        {res.sub_kelas || res.kelas}
                      </span>
                    </td>
                    ...
                    {isWpmMode ? (
                      <>
                        <td className="p-3 font-black text-emerald-400 text-base">
                          🚀 {res.wpm || 0} WPM
                        </td>
                        ...
                        <td className="p-3 text-slate-300 font-mono">
                          {res.keystroke_kasar || 0} / {res.koreksi || 0}
                        </td>
                      </>
                    ) : (
                      /* Mode Kelas 4 */
